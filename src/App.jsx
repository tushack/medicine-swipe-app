...
const AdComponent = () => {
  useEffect(() => {
    const script = document.createElement("script");
    script.src =
      "https://pl28940288.profitablecpmratenetwork.com/2d/8d/95/2d8d95f8de29de69c7cb5e1745ceef2a.js";
    script.async = true;
...
function getDecisionLabel(x, language) {
  if (language === "hi") {
    if (x > 40) return "उपयोगी";
    if (x < -40) return "स्किप";
    return "देखें";
  }
...
export default function MedicineSwipePrototype() {
  ...
  useEffect(() => {
    try {
      if (typeof window !== "undefined") {
        localStorage.setItem("mediswipe-language", language);
      }
    } catch (error) {
      // ignore localStorage write errors
    }
  ...
  const getDisplayType = (item) =>
    language === "hi"
      ? item?.type_hi || item?.type || ""
      : item?.type || item?.type_hi || "";
  ...
  const getDisplayCaution = (item) =>
    language === "hi"
      ? item?.caution_hi || item?.caution || ""
      : item?.caution || item?.caution_hi || "";

  const getDisplayDiseases = (item) => {
    const hiList = normalizeList(item?.diseases_hi);
    const enList = normalizeList(item?.diseases);

    if (language === "hi") {
      return hiList.length > 0 ? hiList : enList;
    }
    return enList.length > 0 ? enList : hiList;
  };
            ...
            <div className="relative mx-auto flex min-h-screen max-w-7xl flex-col px-4 py-8 sm:px-6 lg:flex-row lg:items-start lg:gap-8 lg:px-8">
              <section className="mb-6 flex-1 lg:mb-0 lg:max-w-sm">
                <div className="rounded-[30px] border border-white/70 bg-white/85 p-5 shadow-[0_20px_60px_rgba(16,24,40,0.10)] backdrop-blur">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="rounded-3xl border border-green-100 bg-green-50 p-4">
                      <p className="text-sm text-slate-500">{uiText.useful}</p>
                      <p className="mt-1 text-3xl font-extrabold text-green-700">
                        {liked.length}
                      </p>
                    </div>
                    ...
                    <div className="rounded-3xl border border-orange-100 bg-orange-50 p-4">
                      <p className="text-sm text-slate-500">{uiText.skipped}</p>
                      <p className="mt-1 text-3xl font-extrabold text-orange-700">
                        {passed.length}
                      </p>
                    </div>
                  ...
                  >
                    <Heart className="h-4 w-4" />
                    {showSaved ? uiText.hideSaved : uiText.viewSaved}
                  </button>
                  ...
                  {showSaved && (
                    <div className="mt-4 rounded-3xl border border-green-100 bg-green-50/60 p-4">
                      ...
                      {liked.length === 0 ? (
                        <p className="text-sm text-slate-600">{uiText.noSaved}</p>
                      ) : (
                        <div className="max-h-[320px] space-y-3 overflow-y-auto pr-1">
                          {liked.map((item) => (
                            <div
                              ...
                              >
                                ...
                                >
                                  ...
                                  <div className="flex flex-1 flex-col p-5 sm:p-6">
                                    ...
                                    <div className="mb-4 flex flex-wrap gap-2">
                                      {getDisplayDiseases(item).map((disease) => (
                                        <span
                                          key={disease}
                                          className="rounded-full bg-gradient-to-r from-green-100 to-orange-100 px-3 py-2 text-xs font-semibold text-slate-700 shadow-sm"
                                        >
              ...
              <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/50 p-4">
                <div className="relative w-full max-w-md overflow-hidden rounded-[34px] bg-white shadow-[0_24px_70px_rgba(15,23,42,0.25)]">
                  ...
                  <div className="relative h-[280px] w-full">
                    <img
                      src={selectedSaved.image || fallbackImage}
                      alt={getDisplayName(selectedSaved)}
                      className="h-full w-full object-cover"
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/35 via-black/10 to-transparent" />
                    <div className="absolute bottom-4 left-4 right-4 rounded-2xl bg-white/92 p-4 shadow-lg backdrop-blur-md">
                      <div className="mb-2 flex items-center justify-between gap-3">
                        <h2 className="text-xl font-extrabold tracking-tight text-slate-900 sm:text-2xl">
                          {getDisplayName(selectedSaved)}
                        </h2>
                        <span className="rounded-full bg-green-100 px-3 py-1 text-xs font-semibold text-green-700">
                          {getDisplayType(selectedSaved)}
                        </span>
                      </div>
                      <p className="text-sm leading-5 text-slate-700">
                        {getDisplaySummary(selectedSaved)}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="p-5 sm:p-6">
                    ...
                    <div className="mb-4 flex flex-wrap gap-2">
                      {getDisplayDiseases(selectedSaved).map((disease) => (
                        <span
                          key={disease}
                          className="rounded-full bg-gradient-to-r from-green-100 to-orange-100 px-3 py-2 text-xs font-semibold text-slate-700 shadow-sm"
                        >
                    ...
                    <div className="rounded-2xl bg-slate-50 p-4 text-sm leading-6 text-slate-600">
                      ...
                      {getDisplayCaution(selectedSaved)}
                    </div>
